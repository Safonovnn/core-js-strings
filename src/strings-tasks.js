...
function countVowels(str) {
  const transformStr = str.toLowerCase().split('');
  const result = transformStr.filter((char) => 'aeiouy'.includes(char));
...
function encodeToRot13(str) {
  return str.replace(/[A-Za-z]/g, (char) => {
    const offset = char <= 'Z' ? 65 : 97; // Determine if the character is uppercase or lowercase
